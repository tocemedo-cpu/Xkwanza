...
| Categorias | `/admin/categorias` | Criar, renomear e remover categorias do catálogo (`POST`/`PATCH`/`DELETE /api/categories`). Não há seed automático — uma base de dados nova (incluindo produção) começa sem nenhuma categoria, e o selector de categoria no formulário de publicação fica vazio até um ADMIN criar pelo menos uma aqui. O botão "Criar categorias padrão" (`POST /api/categories/seed-defaults`, idempotente) cria de uma vez uma lista inicial razoável para produtos e serviços. |
